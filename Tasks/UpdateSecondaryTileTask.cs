    ...
    */
    public sealed class UpdateSecondaryTileTask : IBackgroundTask
    {
        ...
        public async void Run(IBackgroundTaskInstance taskInstance)
        {
            ...
            //允许自动更新
            if (getUserRespose.UserConfig.IsWifiAutoUpdate == 1)
            {
                ...
                if (getUserRespose.UserConfig.IsAutoUpdateForCities == 1)
                {
                    foreach (var item in getUserCityRespose.UserCities)
                    {
                        ...
                        UpdateSecondaryTile(item.CityId + "_Weather", item.CityName, getWeatherRespose);
                ...
                else//允许更新默认城市
                {
                    ...
                    if (userCity != null)
                    {
                        UpdateSecondaryTile(tileId, userCity.CityName, getWeatherRespose);
                    }
        ...
        private static void UpdateSecondaryTile(string tileId, string cityName, GetWeatherRespose respose)
        {
            string tileXmlString = @"<tile>"
                + "<visual version='2'>"
                + "<binding template='TileWide310x150BlockAndText01' fallback='TileWideBlockAndText01'>"
                + "<text id='1'>湿度：" + respose.result.sk.humidity + "</text>"
                + "<text id='2'>风向：" + respose.result.sk.wind_direction + "</text>"
                + "<text id='3'>风力：" + respose.result.sk.wind_strength + "</text>"
                + "<text id='4'>湿度：" + respose.result.sk.time + "</text>"
                + "<text id='5'>" + cityName + "</text>"
                + "<text id='6'>" + respose.result.sk.temp + "°</text>"
                + "</binding>"
                + "<binding template='TileSquare150x150PeekImageAndText01' fallback='TileSquarePeekImageAndText01'>"
                + "<image id='1' src='ms-appx:///Assets/Logo.scale-100.png' alt='alt text'/>"
                + "<text id='1'>气温：" + respose.result.sk.temp + "°</text>"
                + "<text id='2'>风向：" + respose.result.sk.wind_direction + "</text>"
                + "<text id='3'>风力：" + respose.result.sk.wind_strength + "</text>"
                + "<text id='4'>湿度：" + respose.result.sk.time + "</text>"
                + "</visual>"
